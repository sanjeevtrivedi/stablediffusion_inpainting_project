...
Splits images in data/samples into train/val/test subsets, generates
pre-saved center and irregular masks for each image, and writes a
manifest CSV for reproducible experiments.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Prepare dataset splits and pre-saved masks")
    ...
    for split_name, split_files in splits.items():
        for mask_type in MASK_TYPES:
            ...
            for src_path in split_files:
                image = Image.open(src_path).convert("RGB").resize(
